  ...
  return (
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10">
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="tag bg-blue-500/10 text-blue-300 border border-blue-500/20 mb-4 inline-block">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-slate-400">Real products built by DevNation members — from hackathon prototypes to production-ready apps.</p>
            </div>
            {isAdmin && (
        ...
        <div className="flex flex-col gap-4 mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map(c => (
              <button key={c} onClick={() => setCategory(c)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${category === c ? 'bg-brand-600 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>{c}</button>
            ))}
            <div className="w-px bg-white/10 mx-1" />
            {['All', 'Active', 'Completed', 'Planning'].map(s => (
              <button key={s} onClick={() => setStatus(s)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${status === s ? 'bg-brand-600 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>{s}</button>
            ))}
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((project, i) => (
              <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-card rounded-2xl overflow-hidden flex flex-col">
                <div className="h-48 overflow-hidden relative">
                  <img src={project.image} alt={project.title} className="w-full h-full object-cover" />
                  {project.featured && (
                    <div className="absolute top-3 left-3 tag bg-amber-500/20 text-amber-300 border border-amber-500/30 backdrop-blur-sm">Featured</div>
                  )}
                  <div className="absolute top-3 right-3 flex gap-2">
                    <span className={`tag backdrop-blur-sm ${statusColors[project.status]}`}>{project.status}</span>
                  </div>
                </div>
                <div className="p-5 flex-1 flex flex-col">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <span className="tag bg-white/5 text-slate-400 text-xs">{project.category}</span>
                    {isAdmin && (
                      <div className="flex gap-1 shrink-0">
                        <button onClick={() => { setEditing(project); setShowModal(true) }} className="p-1.5 rounded-lg text-slate-500 hover:text-white hover:bg-white/10 transition-all"><Pencil className="w-3.5 h-3.5" /></button>
                        <button onClick={() => handleDelete(project.id)} className="p-1.5 rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {project.techStack.map(t => <span key={t} className="tag bg-brand-500/10 text-brand-300 text-xs">{t}</span>)}
                  </div>
                  <div className="text-slate-500 text-xs mb-4">Team: {project.team.join(', ')}</div>
                  <div className="flex gap-3">
                    {project.githubLink && (
                      <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-slate-400 hover:text-white text-sm font-medium transition-colors">
                        <Github className="w-4 h-4" /> GitHub
                      </a>
                    )}
                    {project.liveLink && (
                      <a href={project.liveLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-brand-300 hover:text-brand-200 text-sm font-medium transition-colors">
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center modal-overlay p-4">
      <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-[#0f0f2a] border border-white/10 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-display font-bold text-white">{project ? 'Edit Project' : 'Add Project'}</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <input type="checkbox" id="featured" checked={form.featured || false} onChange={e => setForm(p => ({ ...p, featured: e.target.checked }))} className="w-4 h-4 rounded border-white/20 bg-white/5" />
            <label htmlFor="featured" className="text-slate-300 text-sm cursor-pointer">Featured project</label>
          </div>
        </div>
